...
const render = (content) =>
  `<html>
   <header>
     <title>TTN demo 🚀</title>
   </header>
   <body style="text-align: center; font-family: 'Source Sans Pro';">
     <h1 style="margin-top: 2rem">TTN demo 🚀</h1>
...
app.get("/", (req, res) => {
  if (values.length === 0) {
    return res.send(
      render('<h2 style="margin-top: 2rem">No data received yet 🙁</h2>')
    );
  }

  res.send(render('<h2 style="margin-top: 2rem;">Loading data…</h2>'));
});
...
app.post("/webhooks", (req, res) => {
  ...
  //     isOn
  //       ? new Uint8Array([255, 255, 255, 255, 0])
  //       : new Uint8Array([255, 255, 255, 0, 255])
  //   ).toString("base64");
